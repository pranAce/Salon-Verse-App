...
class _SalonAdminStaffState extends State<SalonAdminStaff> {
  ...
  Future<void> _toggleAvailability(BuildContext context, String salonId, StaffModel staff) async {
    final workspaceProvider = Provider.of<SalonWorkspaceProvider>(context, listen: false);
    ...
    try {
      ...
      if (context.mounted) {
        if (success) {
          ...
          AppFeedback.error(context, workspaceProvider.error ?? "Failed to update availability.");
        }
      }
    } catch (e) {
      if (context.mounted) {
    ...
    return Scaffold(
      ...
      body: Column(
        ...
        children: [
          ...
          Expanded(
            child: workspaceProvider.isLoading && workspaceProvider.staffList.isEmpty
                    ...
                    ? const Center(
                        child: Column(
                          ...
                          children: [
                            ...
                            SizedBox(height: 12),
                            Text("No staff registered yet.", style: TextStyle(fontWeight: FontWeight.bold, color: Colors.grey)),
                          ],
                        ),
                      )
                    : ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          // Stylists can only toggle their OWN availability. Admins can toggle anyone.
                          final canToggle = !isStaff || (isStaff && (s.id == user?.id || s.email.toLowerCase() == user?.email.toLowerCase()));
                                  ...
                                  Column(
                                    ...
                                    children: [
                                      Switch.adaptive(
                                        ...
                                        onChanged: (_) => _toggleAvailability(context, salonId, s),
                                      ),
                                      Text(
